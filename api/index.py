...
def unique():
    ...
    # 集計結果をCSVとしてダウンロード
@app.route('/api/download_aggregate', methods=['POST'])
def download_aggregate():
    ...
    final_agg = combined_results_df.groupby('品種')['分量_kg'].sum().reset_index()
    final_agg.rename(columns={'分量_kg': '合計分量_kg'}, inplace=True)
    # ここに以前の aggregate_uploaded_files()関数のロジックをコピー
    # 最終的なDataFrameを final_agg に代入

    # CSVデータをメモリ上で作成
    csv_buffer = io.StringIO()
    final_agg.to_csv(csv_buffer, index=False, encoding='utf-8-sig')
    csv_buffer.seek(0)
    ...
    return send_file(
        ...
        mimetype='text/csv',
        as_attachment=True,
        download_name='品種別集計結果.csv'
    )

# ユニークな商品名リストをCSVとしてダウンロード
@app.route('/api/download_unique', methods=['POST'])
def download_unique():
    uploaded_files = request.files.getlist('csv_files')
    # ... unique()関数の集計ロジック ...
    """
    ...
    for file in uploaded_files:
        ...
        try:
            ...
            results[file_name] = f"読み込みエラー: {e}"
    # ここに以前の unique()関数のロジックをコピー
    # 最終的な辞書を results に代入

    # 辞書をDataFrameに変換
    combined_df = pd.DataFrame(columns=['ファイル名', '商品名', 'カウント数'])
    for file_name, counts in results.items():
        if isinstance(counts, dict):
            temp_df = pd.DataFrame(counts.items(), columns=['商品名', 'カウント数'])
            temp_df['ファイル名'] = file_name
            combined_df = pd.concat([combined_df, temp_df[['ファイル名', '商品名', 'カウント数']]], ignore_index=True)
